...
# 'load_nifti.m' was adjusted to gunzip to /tmp                                                                   #
#                                                                                                                 #
...
container_freewater=freewater.sif
apptainer_freewater="apptainer run --cleanenv --no-home --userns \
    -B $PROJ_DIR \
...
CMD_FWE="
   $apptainer_freewater \
   $1 ses-${SESSION} $INPUT_DWI $INPUT_BVAL $INPUT_BVEC $INPUT_MASK $FW_OUTPUT_DIR"
...
$apptainer_fsl /bin/bash -c "$CMD_FWC"
