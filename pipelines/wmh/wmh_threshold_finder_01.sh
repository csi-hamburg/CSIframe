...
if [ $ALGORITHM_COMBI = "single" ]; then 
    ...
    if [ $ALGORITHM == "LOCATE" ]; then 
        ...
        JSON_FILE_dice=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-diceindex_desc-${ALGORITHM}.txt
        JSON_FILE_jaccard=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-jaccard_desc-${ALGORITHM}.txt
        FALSE_POSITIVE=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-falsepositive_desc-${ALGORITHM}.nii.gz
        JSON_FILE_falsepositive=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-falsepositive_desc-${ALGORITHM}.txt
        FALSE_NEGATIVE=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-falsenegative_desc-${ALGORITHM}.nii.gz
        JSON_FILE_falsenegative=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-falsenegative_desc-${ALGORITHM}.txt
        ...
        for thresh in 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45 0.5 0.55 0.6 0.65 0.7 0.75 0.8 0.85 0.9 0.95; do 
            ...
            if [ $ALGORITHM == "bianca" ]; then 
                ...
                # Define outputs
                SEGMENTATION_sized=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-wmh_desc-${ALGORITHM}${thresh}_mask_sized.nii.gz
                SEGMENTATION=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-wmh_desc-${ALGORITHM}${thresh}_mask.nii.gz
                SEGMENTATION_THRESH=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM}${thresh}_mask.nii.gz
                ...
                # Define inputs
                SEGMENTATION=$ALGORITHM_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-maskednothresh_desc-wmh_desc-${ALGORITHM}_mask.nii.gz
            ...
            # Define outputs
            SEGMENTATION_MAN=$MAN_SEGMENTATION_DIR/${1}/anat/${1}_DC_FLAIR_label3.nii.gz
            JSON_FILE_dice=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-diceindex_desc-${ALGORITHM}${thresh}.txt
            JSON_FILE_jaccard=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-jaccard_desc-${ALGORITHM}${thresh}.txt
            FALSE_POSITIVE=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-falsepositive_desc-${ALGORITHM}${thresh}.nii.gz
            JSON_FILE_falsepositive=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-falsepositive_desc-${ALGORITHM}${thresh}.txt
            FALSE_NEGATIVE=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-falsenegative_desc-${ALGORITHM}${thresh}.nii.gz
            JSON_FILE_falsenegative=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-falsenegative_desc-${ALGORITHM}${thresh}.txt
    ...
    for thresh in 0.05 0.1 0.15 0.2 0.25 0.3 0.35 0.4 0.45 0.5 0.55 0.6 0.65 0.7 0.75 0.8 0.85 0.9 0.95; do 
        ...
        if [ $ALGORITHM1 == "bianca" ]; then 
            ...
            SEGMENTATION_raw=$DATA_DIR/$PIPELINE/sourcedata/BIANCA_training/pseudomized_training_masks/${1}_bianca_output.nii.gz
            BRAINWITHOUTRIBBON_MASK_FLAIR=$OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-brainwithoutribbon_mask.nii.gz
            SEGMENTATION_TWO=$SECOND_ALGORITHM_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-maskednothresh_desc-wmh_desc-${ALGORITHM2}_mask.nii.gz

            # Define outputs
            SEGMENTATION_sized=$FIRST_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-wmh_desc-${ALGORITHM1}${thresh}_mask_sized.nii.gz
            SEGMENTATION=$FIRST_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-wmh_desc-${ALGORITHM1}${thresh}_mask.nii.gz
            SEGMENTATION_FILTERED=$FIRST_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM1}${thresh}_mask.nii.gz
            SEGMENTATIONTWO_THRESH=$SECOND_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM2}${thresh}_mask.nii.gz
            ...
            # Define inputs
            SEGMENTATION_ONE=$FIRST_ALGORITHM_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-maskednothresh_desc-wmh_desc-${ALGORITHM1}_mask.nii.gz
            SEGMENTATION_raw=$DATA_DIR/$PIPELINE/sourcedata/BIANCA_training/pseudomized_training_masks/${1}_bianca_output.nii.gz
            BRAINWITHOUTRIBBON_MASK_FLAIR=$OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-brainwithoutribbon_mask.nii.gz

            # Define outputs
            SEGMENTATIONONE_THRESH=$FIRST_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM1}${thresh}_mask.nii.gz
            SEGMENTATION_sized=$SECOND_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-wmh_desc-${ALGORITHM2}${thresh}_mask_sized.nii.gz
            SEGMENTATION=$SECOND_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-wmh_desc-${ALGORITHM2}${thresh}_mask.nii.gz
            SEGMENTATION_FILTERED=$SECOND_ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM2}${thresh}_mask.nii.gz
    ...
    for MASKONE in $(ls $FIRST_ALGORITHM_DIR_ITER/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM1}* -d | xargs -n 1 basename); do

        MASKONE_FILE=$FIRST_ALGORITHM_OUT_DIR/$MASKONE
        [ $ALGORITHM1 == "LOCATE" ] && MASKONE_FILE=$FIRST_ALGORITHM_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM1}_mask.nii.gz
        MASKONE_name=$(echo $MASKONE | sed -n "s/.*${ALGORITHM1}//p" | sed -n 's/.nii.gz//p' )
        [ $ALGORITHM1 == "LOCATE" ] && MASKONE_name=""

        for MASKTWO in $(ls $SECOND_ALGORITHM_DIR_ITER/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM2}* -d | xargs -n 1 basename); do

            MASKTWO_FILE=$SECOND_ALGORITHM_OUT_DIR/$MASKTWO
            [ $ALGORITHM2 == "LOCATE" ] && MASKTWO_FILE=$SECOND_ALGORITHM_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM2}_mask.nii.gz
            ...
            # Define outputs
            COMBINATION=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-wmh_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.nii.gz
            JSON_FILE_dice=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-diceindex_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.txt
            JSON_FILE_jaccard=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-jaccard_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.txt
            FALSE_POSITIVE=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-falsepositive_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.nii.gz
            JSON_FILE_falsepositive=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-falsepositive_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.txt
            FALSE_NEGATIVE=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_space-FLAIR_desc-masked_desc-falsenegative_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.nii.gz
            JSON_FILE_falsenegative=$ALGORITHM_OUT_DIR/${1}_ses-${SESSION}_desc-falsenegative_desc-${ALGORITHM1}${MASKONE_name}X${ALGORITHM2}${MASKTWO_name}.txt
